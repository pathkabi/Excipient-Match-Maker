...
if not st.session_state.show_results:
    ...
    # --- Add a new excipient ---
    st.markdown("---")
    st.subheader("Submit a new excipient not found in the list:")
    new_excipient = st.text_input("Enter new excipient name:")
    if st.button("Submit") and new_excipient:
        new_excipient_clean = new_excipient.strip()
        if new_excipient_clean not in st.session_state["existing_select"]:
            st.session_state["existing_select"].append(new_excipient_clean)
            st.success(f"{new_excipient_clean} added.")
        else:
            st.warning(f"{new_excipient_clean} is already in the list.")
...
# === RESULTS SECTION (simulated new page) ===
    ...
    with left:
        ...
        if not st.session_state.final_excipients:
            ...
            for (pair, severity) in st.session_state.issues:
                st.markdown(f"- **{pair[0]}** & **{pair[1]}** → *{severity} incompatibility*")
    ...
    with col1:
        ...
        def draw_compatibility_graph(excipients):
            ...
            # Improved layout for better spacing and less overlap
            # k: smaller k brings nodes closer. Adjust this carefully to avoid overlap.
            # Reduced k slightly from previous version to bring nodes closer.
            num_nodes = len(G.nodes())
            # A smaller k value for bringing nodes closer, but still dynamic
            ...
            plt.figure(figsize=(fig_width, fig_height))  # Bigger canvas for larger text
            node_size = 5000
            nx.draw(
            ...
            font_size = 14 # Fixed at 14 for better visibility with larger nodes
            if num_nodes > 5: # Adjust font size down for very large graphs
            ...
            edge_label_font_size = max(9, 14 - num_nodes) # Slightly larger minimum for edge labels
